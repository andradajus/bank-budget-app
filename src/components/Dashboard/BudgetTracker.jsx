...
const BudgetTracker = ({user, updateBalances, addTransactionToHistory, showAlert}) => {
  ...
  return (
    <div>
      ...
      <div className="mt-2 mb-2">
        ...
        />
        <button className="bg-blue-500 hover:bg-blue-700 ml-1 px-1 py-1 text-white font-bold rounded focus:outline-none focus:shadow-outline" onClick={handleAddExpense}>
  ...
  {expenses.map((expense, index) => (
    <div key={index} className="flex justify-between items-center py-2 ">
      ...
      <span className= "bg-blue-200 rounded-md ml-1 w-36 text-center text-m font-bold text-gray-900 dark:text-white">{expense.name}</span>
      <span className="bg-blue-200 rounded-md ml-1 w-36 text-center font-semibold text-slate-800 text-m lining-nums">&#x20B1;{expense.amount}</span>
      ...
      <div className="flex justify-center">
       <button className="mt-2 bg-blue-500 hover:bg-blue-700 ml-1 px-2 py-2 text-white font-bold rounded focus:outline-none focus:shadow-outline" onClick={handleSubmit}>Submit</button>
      </div>
    </div>
  );
};
