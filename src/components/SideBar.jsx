...
const SideBar = () => {
  ...
  return (
    <>
      <Card className="fixed top-0 left-0 h-screen w-20 m-0 flex flex-col bg-gradient-to-l from-blue-800 via-blue-900 to-blue-800  text-white shadow-2xl rounded ">
...
const SideBarLink = ({ icon, text, to }) => {
  ...
  return (
    ...
    >
      ...
      {isTooltipVisible && (
        <CardHeader className="absolute left-16 top-4 p-2 min-w-max rounded-md shadow-md text-white bg-blue-500 text-xs font-bold transition-all duration-100">
          {text}
        </CardHeader>
      )}
    </div>
